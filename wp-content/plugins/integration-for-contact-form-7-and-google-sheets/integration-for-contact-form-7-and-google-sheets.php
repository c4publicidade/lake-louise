...
* Author: CRM Perks
* Text Domain: integration-for-contact-form-7-and-spreadsheets
...
if( !class_exists( 'vxcf_googlesheets' ) ):

class vxcf_googlesheets {
  ...
  public static $title='Contact Form Google Sheets Plugin';  
  public static $path = ''; 
  public static $slug = "";
...
self::$plugin->instance();
  ...
  */
  ...
  if($type == "admin"){
     if($ver<4.2){
  ?>
    <div class="error vx_notice notice" data-id="<?php echo esc_attr($id) ?>"><p style="display: table"><span style="display: table-cell; width: 98%"><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Google Sheets Plugin','integration-for-contact-form-7-and-spreadsheets') ?>. </b><?php echo wp_kses_post($message);?> </span>
<span style="display: table-cell; padding-left: 10px; vertical-align: middle;"><a href="#" class="notice-dismiss" title="<?php esc_html_e('Dismiss Notice','integration-for-contact-form-7-and-spreadsheets') ?>">dismiss</a></span> </p></div>
  <?php
     }else{
  ?>
  <div class="error vx_notice notice below-h2 is-dismissible" data-id="<?php echo esc_attr($id) ?>"><p><span class="dashicons dashicons-megaphone"></span> <b><?php esc_html_e('Contact Form Google Sheets Plugin','integration-for-contact-form-7-and-spreadsheets') ?>. </b> <?php echo wp_kses_post($message);?> </p>
  </div>    
  <?php
     }
  }else{
...
*/
  ...
  $message = sprintf(__("%sContact Form Entries%s is required. %s it today!%s", 'integration-for-contact-form-7-and-spreadsheets'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");   
  }else if($installed === 2){ // not active
  $message = sprintf(__('Contact Form Entries is installed but not active. %sActivate Contact Form Entries%s to use the Contact Form Google Sheets Plugin','integration-for-contact-form-7-and-spreadsheets'), '<strong><a href="'.wp_nonce_url(admin_url('plugins.php?action=activate&plugin=contact-form-entries-pro/contact-form-entries-pro.php'), 'activate-plugin_contact-form-entries-pro/contact-form-entries-pro.php').'">', '</a></strong>');  
  } else if($installed === 3){ // not supported
  $message = sprintf(__("A higher version of %sContact Form Entries%s is required. %sPurchase it today!%s", 'integration-for-contact-form-7-and-spreadsheets'), "<a href='https://www.crmperks.com/'>", "</a>", "<a href='https://www.crmperks.com/'>", "</a>");
  }  
...
$value=trim(implode(' ',$value));
 }else if( is_array($value)  ){ //&& count($value) == 1  ,  removed at May-2024 , google does not accept empty checkbox array
   $value=trim(implode(', ',$value)); //add comma between checkbox values   
 }
 if($type == 'free'){$value='';}
  return $value;        
}
  ...
  */
  ...
  $link="N/A"; $desc=__("Added to ",'integration-for-contact-form-7-and-spreadsheets');
  ...
  if( !empty($feed['object']) && !empty($feed['status'])){
   if($feed['link'] !=""){
      $title='Row #'.$feed['crm_id'];
      if (filter_var($feed['link'], FILTER_VALIDATE_URL) === true) {
      $title=basename($feed['link']);
      }
  $link='<a href="'.$feed['link'].'" title="'.$feed['crm_id'].'" target="_blank">'.$title.'</a>';
  $crm_id=$feed['crm_id'];
  }
  ...
  $desc.=$feed['object'];
  if($feed['status'] == 5){
   $desc=sprintf(__("%s Deleted",'integration-for-contact-form-7-and-spreadsheets'),$feed['object']);   
  }if($feed['status'] == 4){
      $desc=sprintf(__("%s Filtered",'integration-for-contact-form-7-and-spreadsheets'),$feed['object']); 
  }
  }else{
  $desc= !empty($feed['error']) ? $feed['error'] : esc_html__("Unknown Error",'integration-for-contact-form-7-and-spreadsheets');
  }
  $title=__("Failed",'integration-for-contact-form-7-and-spreadsheets');   
  if($feed['status'] == 1){
  $title=__("Created",'integration-for-contact-form-7-and-spreadsheets');   
  }else if($feed['status'] == 2){
  $title=__("Updated",'integration-for-contact-form-7-and-spreadsheets');   
  }else if($feed['status'] == 4){
  $title=__("Filtered",'integration-for-contact-form-7-and-spreadsheets');   
  }else if($feed['status'] == 5){
  $title=__("Deleted",'integration-for-contact-form-7-and-spreadsheets');   
  }
  ...
  */
  ...
  printf('<input type="checkbox" name="'.esc_attr($this->id).'_update" id="'.esc_attr($this->id).'_update" value="1" /><label for="'.esc_attr($this->id).'_update" title="%s">%s</label><br /><br />', esc_html__('Create or update this entry in Google Sheets. The fields will be mapped according to the form feed settings.', 'integration-for-contact-form-7-and-spreadsheets'), esc_html__('Send to Google Sheets', 'integration-for-contact-form-7-and-spreadsheets'));
  } else {
...
if(!current_user_can($this->id."_send_to_crm")){return; }
  ...
  $button.= '<input type="submit" class="button button-large button-secondary alignright" name="'.esc_attr($this->id).'_send" style="margin-left:5px; '.$margin.'" title="'.__('Create or update this entry in Google Sheets. The fields will be mapped according to the form feed settings.','integration-for-contact-form-7-and-spreadsheets').'" value="'.__('Send to Google Sheets', 'integration-for-contact-form-7-and-spreadsheets').'" onclick="jQuery(\'#action\').val(\'send_to_crm\')" />';
  //logs button
    ...
    $button.= '<a class="button button-large button-secondary alignright" style="margin-left:5px; margin-top:5px; " title="'.__('Go to Google Sheets Logs','integration-for-contact-form-7-and-spreadsheets').'" href="'.esc_url($log_url).'">'.__('Google Sheets Logs','integration-for-contact-form-7-and-spreadsheets').'</a>';
...
return  $var;
  ...
  */
  ...
  $status=0;
  if(class_exists('vxcf_googlesheets_wp')){
      $status=1;
  }else if( file_exists(dirname(dirname(__FILE__)).'/integration-for-contact-form-7-and-spreadsheets/integration-for-contact-form-7-and-spreadsheets.php')) {
  $status=2;
  } 
  return $status;
  }
...
public function do_actions(){
  ...
  */
  ...
  $this->data=new vxcf_googlesheets_data();
  return $this->data;
  }
  /**
...
$value=$this->process_tags($entry,$value); 
 ...
 if(is_array($value)){ $value=trim(implode(' ',$value)); }
    if(!empty($value)){ $value='1'; }  //single checkbox or acceptance field , googlesheets web2lead accepts only 1 for checkbox field
  }
  }}
...
$temp=apply_filters('crmperks_googlesheets_post',$temp,$feed,$entry);
  ...
  if( $added){
      ...
      if($action == "Added"){
          if(empty($res['link'])){
  $msg=sprintf(__('Successfully Added to Google Sheets (%s) with ID # %s .', 'integration-for-contact-form-7-and-spreadsheets'),$feed['object'],$res['id']);
          }else{
  $msg=sprintf(__('Successfully Added to Google Sheets (%s) with ID # %s . View entry at %s', 'integration-for-contact-form-7-and-spreadsheets'),$feed['object'],$res['id'],$res['link']);
          }
  $screen_msg=__( 'Entry added in Google Sheets', 'integration-for-contact-form-7-and-spreadsheets');
      }else{
            if(empty($res['link'])){
  $msg=sprintf(__('Successfully Updated to Google Sheets (%s) with ID # %s . View entry at %s', 'integration-for-contact-form-7-and-spreadsheets'),$feed['object'],$res['id'],$res['link']);   
            }else{
  $msg=sprintf(__('Successfully Updated to Google Sheets (%s) with ID # %s .', 'integration-for-contact-form-7-and-spreadsheets'),$feed['object'],$res['id']);   
            }
          if($event == 'delete'){  
     $screen_msg=__( 'Entry deleted from Google Sheets', 'integration-for-contact-form-7-and-spreadsheets');
          }else{
     $screen_msg=__( 'Entry updated in Google Sheets', 'integration-for-contact-form-7-and-spreadsheets');
          }
          }
   
  
  }else if($this->post('status',$res) == 4){
  $screen_msg=$msg=__( 'Entry filtered', 'integration-for-contact-form-7-and-spreadsheets');    
  }else{
  $status=0; $screen_msg_class="error";
  $screen_msg=__('Errors when adding to Google Sheets. Entry not sent! Check the Entry Notes below for more details.' , 'integration-for-contact-form-7-and-spreadsheets' );
  if($log_id!=""){
      //message for  bulk actions in logs
  $screen_msg=__('Errors when adding to Google Sheets. Entry not sent' , 'integration-for-contact-form-7-and-spreadsheets' );    
  }
  $msg=sprintf(__('Error while creating %s', 'integration-for-contact-form-7-and-spreadsheets'),$feed['object']);
...
$this->send_error_email($info,$entry,$form);
  ...
  $log_link=' <a href="'.esc_url($log_url).'" class="vx_log_link" data-id="'.$insert_id.'">'.__('View Detail','integration-for-contact-form-7-and-spreadsheets')."</a>";
 $screen_msg.=$log_link;
    }
    if($post_comment){
...
//$this->add_note($entry["id"], $msg);
  ...
  */
  ...
  $detail["Form Id"]=$form['id'];
    ...
    $email_info=array("msg"=>$info['msg'],"title"=>__('Google Sheets','integration-for-contact-form-7-and-spreadsheets')." Error","info_title"=>"More Detail","info"=>$detail);
...
$vxcf_googlesheets=new vxcf_googlesheets();
$vxcf_googlesheets->instance();
$vx_cf['vxcf_googlesheets']='vxcf_googlesheets';
endif;
